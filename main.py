...
[bold cyan]LocalAI Commands:[/bold cyan]
  ...
  [bold]/download-model[/]- Download the configured Qwen model
...
def handle_command(
    ...
    elif verb in ("/download-model", "/download"):
        requested_model = arg or config.OLLAMA_MODEL
        console.print(f"[dim]Downloading model: {requested_model}[/dim]")
        if llm.download_model(requested_model):
            console.print(f"[green]OK Using model: {llm.model}[/green]")
        else:
            console.print(
                "[red]Model download failed. Check your internet connection and Ollama setup.[/red]"
            )
